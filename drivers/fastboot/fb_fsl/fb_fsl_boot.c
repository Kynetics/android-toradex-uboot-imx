...
int do_boota(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[]) {
	...
	if (is_load_fdt_from_part()) {
		...
		board_name = env_get("board_device_tree");

		if (board_name == NULL)
			goto fail;

		for (i = 0; i < be32_to_cpu(dt_img->dt_entry_count); i++) {
			dt_entry = (struct dt_table_entry *)((ulong)dt_img +
                                be32_to_cpu(dt_img->dt_entries_offset)  + i*be32_to_cpu(dt_img->dt_entry_size));
			...
			int root_node_off = fdt_path_offset((void *)fdt_addr, "/");
			const char *device_tree_name =
				(const char *)fdt_getprop((void *)fdt_addr, root_node_off, "dtb_filename", NULL);

			if (device_tree_name == NULL) {
				printf("Device tree selection not supported: \"dtb_filename\" property not found \n");
				goto fail;
			}

			if (!strcmp( device_tree_name, board_name)) {
				printf("Selected device tree: %s\n", device_tree_name);
		...
		if (!valid_dtb_found) {
			printf ("No compatible device tree found\n");
			goto fail;
		}
	} else {
